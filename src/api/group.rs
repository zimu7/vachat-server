...
/// Create or update announcement request
#[derive(Debug, Object)]
...
impl ApiGroup {
    ...
    /// Clear all information of a group
    #[oai(path = "/:gid/clear", method = "delete", transform = "guest_forbidden")]
    async fn clear(&self, state: Data<&State>, token: Token, gid: Path<i64>) -> Result<()> {
        let mut cache = state.cache.write().await;
        let group = match cache.groups.get(&gid.0) {
            Some(group) => group,
            None => return Err(Error::from_status(StatusCode::NOT_FOUND)),
        ...
        // Collect info before mutating cache
        let pinned_mids: Vec<i64> = group.pinned_messages.iter().map(|m| m.mid).collect();
        let targets: BTreeSet<i64> = if group.ty.is_public() {
            cache.users.keys().copied().collect()
        } else {
            group.members.clone()
        };
        ...
        sqlx::query("delete from pinned_message where gid = ?")
            .bind(gid.0)
            .execute(&mut tx)
            .await
            .map_err(InternalServerError)?;

        sqlx::query("delete from announcement where gid = ?")
            .bind(gid.0)
            .execute(&mut tx)
            .await
            .map_err(InternalServerError)?;

        for mid in &removed_mids {
            sqlx::query("delete from files where mid = ?")
                .bind(mid)
                .execute(&mut tx)
                .await
                .map_err(InternalServerError)?;
        ...
        sqlx::query("delete from read_index where target_gid = ?")
            .bind(gid.0)
            .execute(&mut tx)
            .await
            .map_err(InternalServerError)?;

        sqlx::query("delete from burn_after_reading where target_gid = ?")
            .bind(gid.0)
            .execute(&mut tx)
            .await
            .map_err(InternalServerError)?;

        sqlx::query("delete from pinned_chat where target_gid = ?")
            .bind(gid.0)
            .execute(&mut tx)
            .await
            .map_err(InternalServerError)?;
        ...
        // Update cache
        let group = cache.groups.get_mut(&gid.0).unwrap();
        group.pinned_messages.clear();
        group.announcement = None;

        for user in cache.users.values_mut() {
            user.read_index_group.remove(&gid.0);
            user.burn_after_reading_group.remove(&gid.0);
            user.pinned_chat_group.remove(&gid.0);
        }

        // Broadcast events
        for mid in &pinned_mids {
            let _ = state
                .event_sender
        ...
        if had_announcement {
            let _ = state
                .event_sender
                .send(Arc::new(BroadcastEvent::GroupAnnouncementChanged {
                    gid: gid.0,
                    announcement: None,
                }));
        }
        ...
        match (group.ty.is_public(), req.is_public) {
            (true, false) => {
                ...
                sqlx::query("update `group` set is_public = ?, owner = ? where gid = ?")
                    ...
                    .await
                    .map_err(InternalServerError)?;
                let mut members: BTreeSet<i64> = if !req.members.is_empty() {
    ...
    async fn add_members(
        ...
        match group.ty {
            GroupType::Public => return Err(Error::from_status(StatusCode::FORBIDDEN)),
            GroupType::Private { owner }
                if owner != token.uid && !group.members.contains(&token.uid) && !token.is_admin =>
            {
                return Err(Error::from_status(StatusCode::FORBIDDEN));
            }
            _ => {}
    ...
    /// Get announcement for a group
    #[oai(path = "/:gid/announcement", method = "get")]
    async fn get_announcement(
        &self,
        state: Data<&State>,
        gid: Path<i64>,
    ) -> Result<Json<GetAnnouncementResponse>> {
        let cache = state.cache.read().await;
        let group = cache
            .groups
            .get(&gid.0)
        ...
        Ok(Json(GetAnnouncementResponse {
            announcement: group.announcement.clone(),
        }))
    }

    /// Create or update announcement for a group
    #[oai(
    ...
    async fn create_or_update_announcement(
        ...
        token: Token,
        gid: Path<i64>,
        req: Json<CreateAnnouncementRequest>,
    ) -> Result<Json<GroupAnnouncement>> {
        let mut cache = state.cache.write().await;
        let group = cache
            .groups
            .get_mut(&gid.0)
        ...
        let can_manage = token.is_admin
            || matches!(&group.ty, GroupType::Private { owner } if *owner == token.uid);
        if !can_manage {
            return Err(Error::from_status(StatusCode::FORBIDDEN));
        }

        let now = DateTime::now();
        let existing = group.announcement.is_some();

        if existing {
            sqlx::query("update announcement set content = ?, updated_at = ? where gid = ?")
                .bind(&req.content)
                .bind(now)
                .bind(gid.0)
                .execute(&state.db_pool)
            ...
            sqlx::query(
                "insert into announcement (gid, content, created_by, created_at, updated_at) values (?, ?, ?, ?, ?)",
            )
            .bind(gid.0)
            .bind(&req.content)
            .bind(token.uid)
        ...
        let announcement = group.announcement.clone();
        let announcement = GroupAnnouncement {
            ...
            created_by: announcement
                ...
                .map(|a| a.created_by)
                .unwrap_or(token.uid),
            created_at: announcement.as_ref().map(|a| a.created_at).unwrap_or(now),
            updated_at: now,
        };

        // update cache
        group.announcement = Some(announcement.clone());
    ...
    /// Delete announcement for a group
    #[oai(
        path = "/:gid/announcement",
        method = "delete",
        transform = "guest_forbidden"
...
mod tests {
    ...
    async fn test_crud() {
        ...
        let uid3 = server.create_user(&admin_token, "test3@zimu.pub").await;
    ...
    async fn test_send_to_private_group() {
        ...
        for i in 1..10 {
            ids.push(
                server
                    .create_user(&admin_token, format!("test{}@zimu.pub", i))
                    .await,
            );
            tokens.push(server.login(format!("test{}@zimu.pub", i)).await);
        }
    ...
    async fn test_send_to_public_group() {
        ...
        server.create_user(&admin_token, "user1@zimu.pub").await;
        let token1 = server.login("user1@zimu.pub").await;
        server.create_user(&admin_token, "user2@zimu.pub").await;
