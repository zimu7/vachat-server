...
#[derive(Debug, Object, Clone)]
pub struct GroupAnnouncementChangedEvent {
    pub gid: i64,
...
/// Pin chat target user
#[derive(Debug, Object, Clone)]
pub struct PinChatTargetUser {
    pub uid: i64,
}

/// Pin chat target channel
#[derive(Debug, Object, Clone)]
pub struct PinChatTargetChannel {
    pub gid: i64,
}

/// Pin chat target
#[derive(Debug, Union, Clone)]
pub enum PinChatTarget {
    #[oai(mapping = "uid")]
    User(PinChatTargetUser),
    #[oai(mapping = "gid")]
    Channel(PinChatTargetChannel),
}

/// Pinned chat
#[derive(Debug, Object, Clone)]
pub struct PinChat {
    pub target: PinChatTarget,
...
pub struct UserSettingsMessage {
    ...
    pub pinned_chat_users: Vec<i64>,
    pub pinned_chat_groups: Vec<i64>,
}
...
pub struct UserSettingsChangedMessage {
    ...
    pub burn_after_reading_users: Vec<BurnAfterReadingUser>,
    pub burn_after_reading_groups: Vec<BurnAfterReadingGroup>,
    pub add_pin_chats: Vec<PinChat>,
    pub remove_pin_chats: Vec<PinChatTarget>,
}
...
pub enum Message {
    ...
    #[oai(mapping = "group_announcement_changed")]
    GroupAnnouncementChanged(GroupAnnouncementChangedEvent),
    ...
    pub async fn into_chat_message_content(
        ...
        match self {
            ...
            SendMessageRequest::File(Json(file_info)) => Ok(ChatMessageContent {
                ...
                content_type: "vachat/file".to_string(),
...
pub async fn send_message(state: &State, mut payload: ChatMessagePayload) -> poem::Result<i64> {
    ...
    let mid = match payload.target {
        MessageTarget::User(MessageTargetUser { uid }) => {
            // check block: if target blocked sender, reject
            if let Some(target_user) = cache.users.get(&uid) {
                if let Some(contact) = target_user.contacts.get(&from_uid) {
                    if contact.status == 2 {
                ...
                if user.status == UserStatus::Normal {
                    if let Some(notify_message) = payload.notify_message(&cache, &mentions) {
                        ...
                        let data = serde_json::json!({
                            "vachat_server_id": &key_config.server_id,
                            "vachat_from_uid": from_uid.to_string(),
                            "vachat_to_uid": uid.to_string(),
                        });
...
async fn send_notify(
    _state: State,
    _notify_tokens: Vec<String>,
    _title: String,
    _message: String,
    _data: impl Serialize + Send + Sync + 'static,
) {
