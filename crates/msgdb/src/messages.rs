...
impl<'a> Messages<'a> {
    ...
    /// Fetch all messages with mid > since_mid, returned in ascending order by mid.
    /// This is useful for fetching messages for multiple users/bots in a single call.
    pub fn fetch_messages_after(
        &self,
        since_mid: i64,
        limit: usize,
    ) -> Result<Vec<(i64, Vec<u8>)>> {
        let start = since_mid + 1;
        let iter = self
            .db
            .db
            .range(key_msg(start)..key_msg(i64::MAX))
            .take(limit);
        let mut msgs = Vec::new();

        for item in iter {
            let (key, value) = item?;
            let msg_id = match decode_key_msg(&key) {
                Some(id) => id,
                None => break,
            };
            msgs.push((msg_id, value.to_vec()));
    ...
    /// Fetch all messages in reverse order (newest first), scanning from before_mid downwards.
    pub fn fetch_messages_before_rev(
        &self,
        before: i64,
        limit: usize,
    ) -> Result<Vec<(i64, Vec<u8>)>> {
        let end_key = key_msg(before);
        let iter = self
            .db
            .db
            .range(key_msg(0)..=end_key)
            .rev()
            .take(limit);
        let mut msgs = Vec::new();
        for item in iter {
    ...
    /// Clear all messages for a group. Returns the list of message IDs that were removed.
    pub fn clear_group_messages(&self, gid: i64) -> Result<Vec<i64>> {
        let mut batch = Batch::default();
        let mut removed_mids = Vec::new();
        let mut removed_mid_set = HashSet::new();

        let start = key_group_msg(gid, 0);
        let end = key_group_msg(gid, i64::MAX);
        let iter = self.db.db.range(start..end);

        for item in iter {
            let (key, _) = item?;
            let (current_gid, mid) = match decode_key_group_msg(&key) {
                Some(res) => res,
                None => break,
            };
            if current_gid != gid {
            ...
            removed_mids.push(mid);
            removed_mid_set.insert(mid);
            batch.remove(&key_msg(mid));
            batch.remove(&key_merged_msg(mid));
            batch.remove(key.as_ref());
        }

        // Remove UMSG/ entries for all users referencing these message IDs
        let umsg_prefix = b"UMSG/";
        for item in self.db.db.scan_prefix(umsg_prefix) {
            let (key, _) = item?;
            let (uid, mid) = match decode_key_user_msg(&key) {
                Some(res) => res,
                None => continue,
            };
            if removed_mid_set.contains(&mid) {
                batch.remove(&key_user_msg(uid, mid));
            }
        }
...
fn decode_key_msg(data: &[u8]) -> Option<i64> {
    let data = data.strip_prefix(b"MSG/")?;
    if data.len() != 8 {
        return None;
    }
    let msg_id = i64::from_be_bytes(data[0..8].try_into().unwrap());
    Some(msg_id)
}
